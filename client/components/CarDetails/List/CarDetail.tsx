...
export default function CarDetail({ carId }: CarDetailProps) {
  ...
  const handleBooking = () => {
    ...
    router.push(
      `/form?title=${encodeURIComponent(car.title)}&price=${encodeURIComponent(
        car.price
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-4">
            {car.images.map((imgUrl, index) => (
              <div
                key={index}
                className="w-full h-24 sm:h-32 md:h-40 relative cursor-pointer border-2 rounded-md overflow-hidden"
                onClick={() =>
                  setSelectedImage(imgUrl || "/cars/placeholder.jpg")
                }
              >
                <Image
